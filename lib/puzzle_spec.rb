    ...
    def spec_source(year, day)
      ...
      RSpec.describe Year#{year}::Day#{day} do
        it "solves part1" do
          ...
          expect(d.part1(PuzzleInput.load("#{year}", "#{day}"))).to eq(Year#{year}::Day#{day}::PART1_ANSWER)
        end

        it "solves part2" do
          d = Year#{year}::Day#{day}.new
          expect(d.part2(PuzzleInput.load("#{year}", "#{day}"))).to eq(Year#{year}::Day#{day}::PART2_ANSWER)
        end
      end
      TPL
    end
